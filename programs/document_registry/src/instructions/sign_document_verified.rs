...
impl HumanProfile {
    pub const DISCRIMINATOR: [u8; 8] = [32, 133, 87, 162, 100, 194, 215, 212];
}
...
pub fn handler(ctx: Context<SignDocumentVerified>, params: SignDocumentTxParams) -> Result<()> {
    ...
    signature.attestation_id = human_profile_info.key();
    ...
    emit!(VerifiedDocumentSigned {
        ...
        human_profile: human_profile_info.key(),
...
pub struct SignDocumentVerified<'info> {
    ...
    /// CHECK: Cross-program account - validated via seeds, owner, and discriminator
    #[account(
        seeds = [b"human_profile", signer.key().as_ref()],
        bump,
        seeds::program = human_registry_program.key(),
        owner = human_registry_program.key()
    )]
    pub human_profile: AccountInfo<'info>,

    #[account(mut, seeds = [b"document", document.doc_hash.as_ref()], bump = document.bump)]
    pub document: Account<'info, Document>,

    #[account(
        init, payer = signer, space = SignatureRecord::LEN,
        seeds = [b"signature", document.key().as_ref(), signer.key().as_ref(), params.role.as_ref()],
        bump
    )]
    pub signature_record: Account<'info, SignatureRecord>,

    #[account(
        init, payer = signer, space = DocumentSigningReceipt::LEN,
        seeds = [b"signing_receipt", document.key().as_ref(), signer.key().as_ref(), params.role.as_ref()],
        bump
    )]
    pub signing_receipt: Account<'info, DocumentSigningReceipt>,

    /// CHECK: Verified by known program ID
    #[account(address = HUMAN_REGISTRY_PROGRAM_ID)]
    pub human_registry_program: AccountInfo<'info>,
