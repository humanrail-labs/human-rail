...
exports.HumanRailAgent = void 0;
const web3_js_1 = require("@solana/web3.js");
const client_js_1 = require("./client.js");
const instructions_js_1 = require("./instructions.js");
const constants_js_1 = require("./constants.js");
class HumanRailAgent {
    constructor(config) {
        ...
        this.client = new client_js_1.HumanRailClient({ rpcUrl, commitment });
    }
    get pubkey() {
    ...
    async checkCapability(params) {
        ...
        for (const cap of status.capabilities) {
            ...
            if (cap.expiresAt > 0 && cap.expiresAt < now)
                continue;
            if ((cap.allowedPrograms & requiredScope) === BigInt(0))
                continue;
    ...
    async getStatus() {
        const status = await this.client.getAgentStatus(this.pubkey);
        const dailyRemaining = status.capabilities.reduce((sum, c) => sum + (c.dailyLimit - c.dailySpent), BigInt(0));
    ...
    async executePayment(params) {
        ...
        const ix = (0, instructions_js_1.buildDirectPaymentIx)({
            from: this.pubkey,
            to: params.to,
    ...
    async signDocument(params) {
        ...
        try {
            const ix = (0, instructions_js_1.buildSignDocumentIx)({
                signer: this.pubkey,
    ...
    // === Event listening ===
    onFreeze(callback) {
        const [freezePda] = (0, constants_js_1.deriveFreezePda)(this.principalPubkey, this.pubkey);
    ...
    async emitReceipt(params) {
        ...
        const ix = (0, instructions_js_1.buildCreateReceiptIx)({
            agent: this.pubkey,
    ...
    async findOrRegisterDocument(docHash) {
        const [documentPda] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("document"), docHash], constants_js_1.PROGRAM_IDS.documentRegistry);
...
exports.HumanRailAgent = HumanRailAgent;
