...
export function useAgentReceipts(agentId: PublicKey | null) {
  ...
  const fetchReceipts = useCallback(async () => {
    ...
    try {
      ...
      const receiptAccounts = await connection.getProgramAccounts(programId, {
        filters: [{ memcmp: { offset: 8 + 32, bytes: agentId.toBase58() } }],
      });
      ...
      console.error("Failed to fetch agent receipts:", err);
      setError(err instanceof Error ? err.message : "Failed to fetch receipts");
    } finally {
      setLoading(false);
    }
  }, [agentId, connection, cluster]);
